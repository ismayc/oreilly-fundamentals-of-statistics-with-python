...
cp coffee_quality.csv data_dev_survey.csv live/
...
# No shell in the in-browser kernel; comment out the !pip cell (packages auto-load on import)
perl -0777 -i -pe 's/!pip install/# (no pip needed in this in-browser kernel) pip install/g' \
...
"$PYBIN" scripts/clean-notebooks.py \
  jupyterlite/contents/exercises.ipynb jupyterlite/contents/exercises_solutions.ipynb
cp coffee_quality.csv data_dev_survey.csv jupyterlite/contents/
